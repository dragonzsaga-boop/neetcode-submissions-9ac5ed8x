class Solution {
    ...
    void backtrack(std::string current, int open, int close, 
                int maxv, std::vector<std::string>& result) {
        if (current.length() == maxv * 2) {
            result.push_back(current);
            return;
        }
        if (open < maxv) {
            backtrack(current + "(", open + 1, close, maxv, result);
        }
        if (close < open) {
            backtrack(current + ")", open, close + 1, maxv, result);
        }
    }
public:
